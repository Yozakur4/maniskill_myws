...
class OpenSafeDoor2Env(BaseEnv):
    ...
    def compute_dense_reward(self, obs: Any, action: torch.Tensor, info: dict):
        # Stage 1: Reach handle — TCP approaches bar handle link
        handle_link = self.safe.links_map[self.HANDLE_JOINT_NAME]
        handle_pos = handle_link.pose.p                              # (B, 3)
        tcp_to_handle = torch.norm(self.agent.tcp.pose.p - handle_pos, dim=-1)
        reach_reward = 1.0 - torch.tanh(5.0 * tcp_to_handle)

        # Stage 2a: Handle turn progress (linear, clipped to [0,1])
        handle_angle = info["handle_angle"]                          # (B,)
        handle_threshold = 1.2  # rad, matches evaluate() logic
        handle_progress = torch.clamp(handle_angle / handle_threshold, 0.0, 1.0)

        # Stage 2b: Button press progress (linear, clipped to [0,1])
        button_threshold = 0.04  # metres
        button_progress = torch.clamp(info["button_qpos"] / button_threshold, 0.0, 1.0)

        # Unlock signal: either handle twisted OR button pressed
        unlock_score = torch.clamp(handle_progress + button_progress, 0.0, 1.0)

        # Stage 3: Door open — sigmoid gate on unlock_score so door reward
        unlock_gate = torch.sigmoid(10.0 * (unlock_score - 0.8))
        door_ratio = info.get(
            "door_open_ratio",
            torch.zeros(self.num_envs, device=self.device),
        )
        open_reward = unlock_gate * door_ratio

        # Weighted sum: reaching + unlocking (handle / button) + door opening
        reward = (
            1.0 * reach_reward
            + 2.0 * handle_progress
            + 2.0 * button_progress
        ...
        # Success bonus: +10 one-shot jump when task is truly complete
