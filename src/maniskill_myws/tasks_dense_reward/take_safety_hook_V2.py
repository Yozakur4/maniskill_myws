...
class TakeSafetyHookEnv(BaseEnv):
    ...
    def evaluate(self):
        ...
        current_success = progress >= torch.tensor(0.75, device=self.device)

        # Latch success permanently within episode
        ...
        return {
            ...
            "progress": progress,
            "hook_qpos": gate_qpos,
            "hook_pos": self.hook.pose.p,
            "beam_pos": self.beam.pose.p,
        }
    ...
    def compute_dense_reward(self, obs: Any, action: torch.Tensor, info: dict):
        ...
        hook_pos = info["hook_pos"]   # (B, 3) 动态获取
        beam_pos = info["beam_pos"]   # (B, 3) 动态获取

        # Stage 1: Reach hook
        dist_tcp_hook = torch.norm(tcp_pos - hook_pos, dim=-1)
        reach_reward = 1.0 - torch.tanh(5.0 * dist_tcp_hook)

        # Stage 2: Lift hook — 平滑门控，0.20→0.24 范围内过渡
        hook_z = hook_pos[:, 2]
        lift_score = torch.sigmoid(20.0 * (hook_z - 0.22))

        # Stage 3: Approach beam — lift 门控
        dist_hook_beam = torch.norm(hook_pos - beam_pos, dim=-1)
        approach_reward = lift_score * (1.0 - torch.tanh(3.0 * dist_hook_beam))

        # Stage 4: Gate progress (hook 挂上 beam 后 gate_joint 被撑开)
        progress = info["progress"]   # [0, 1]
        gate_reward = progress

        # Success bonus: 在 progress=0.7 附近平滑激活，与 success 阈值 0.75 对齐
        success_bonus = 10.0 * torch.sigmoid(8.0 * (progress - 0.7))
